...
static void keyWithMode(FoConsole* console, int vKey, int keyTryer)
{
    ...
    if (console->focusTarget == FOCUS_TEXTAREA || console->focusTarget == FOCUS_TEXTAREA_SELECTION)
    {
...
static bool isFunctionKey(int key)
{
    ...
    if (key == KEY_PPAGE || key == KEY_NPAGE || key == KEY_FOLIC_MARK || 
...
void singleKeyProcess(FoConsole* console, int key)
{
    ...
    else if (isFunctionKey(key)) //if is a functional key
    {
...
void functionKey(FoConsole* console, int key)
{
    switch (key)
    {
        ...
        case KEY_CTRL('c'):
            ...
            break;
        case KEY_CTRL('v'):
            prepareInputStr(console, INPUTSTR_APPEND_SEND, getStrFromClipboard(console->clipBoard));
            break;
        case KEY_CTRL('q'):
        ...
        case KEY_PPAGE: //page up
            textAreaPageUp(console->textArea);
            break;
        case KEY_NPAGE: //page down
            textAreaPageDown(console->textArea);
            break;
        case KEY_FOLIC_MARK:
            if (console->focusTarget == FOCUS_TEXTAREA)
            {
